...
type ITaskDo interface {
	...
	QueryTaskByAttr(attrs []example.Attr) (result []model.Task, err error)
}
...
//
// {{ end }}
func (t taskDo) QueryTaskByAttr(attrs []example.Attr) (result []model.Task, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	generateSQL.WriteString("JOIN (SELECT cea.biz_id task_id ")
	for _, attr1 := range attrs {
		params = append(params, attr1.Code)
		generateSQL.WriteString(", max(if(cea.code = ?, cea.value, NULL)) AS " + t.Quote(attr1.Code) + " ")
	}
	generateSQL.WriteString("FROM colab_entity_attribute cea WHERE cea.biz = 'task' GROUP BY task_id) attrs ON t.id = attrs.task_id ")
	generateSQL.WriteString("WHERE t.deleted_at IS NULL ")
	if len(attrs) > 0 {
		for _, attr2 := range attrs {
			params = append(params, attr2.Value)
			generateSQL.WriteString("AND attrs." + t.Quote(attr2.Code) + " = ? ")
		}
	}
